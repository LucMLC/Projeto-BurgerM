...
const completePurchases = () => {
    completePurchase.addEventListener('click', () => {
        ...
        //Chama a função de fechar o carrinho
        backCart()
    });
};
...
for (let i = 0; i < comprar.length; i++) {
    comprar[i].addEventListener('click', function (event) {
        ...
        // Cria uma li para o novo produto
        ...
        // Adiciona evento de remoção novo botão de remoção
        const newRemoveButton = newProductLi.querySelector('.remover')
        newRemoveButton.addEventListener('click', function () {
            newProductLi.remove()
            calculateDesconto(-productsPrice) // Remover o desconto quando o produto é removido
